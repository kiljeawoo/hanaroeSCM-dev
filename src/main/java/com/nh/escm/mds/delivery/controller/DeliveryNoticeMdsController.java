package com.nh.escm.mds.delivery.controller;
...
import com.nh.escm.mds.delivery.helper.VOConvertHelper;
import com.nh.escm.mds.delivery.service.DeliveryNoticeMdsService;
 ...
 */
@RestController
public class DeliveryNoticeMdsController extends AbstractController {
	...
	@Autowired
	private DeliveryNoticeMdsService deliveryNoticeMdsService;
	 ...
	 * 기    능 : 영업가능일
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveTC")
	public void retrieveTC(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<DeliveryTcVO> list = deliveryNoticeMdsService.retrieveTC(trplC);
	 ...
	 * 기    능 : 발주 정보 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getOrder")
	public void getOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			NoticeOrderVO vo = deliveryNoticeMdsService.getOrder(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpNo);
			...
			//발주상세정보
			List<NoticeOrderDetailVO> list = deliveryNoticeMdsService.getOrderDetail(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpNo);
			if(list != null){
	 ...
	 * 기    능 : 발주 정보 상세조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getOrderDetail")
	public void getOrderDetail(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			//발주상세정보
			List<NoticeOrderDetailVO> list = deliveryNoticeMdsService.getOrderDetail(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpNo, dvyPlaDt);
			if(list != null){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
				dsHelper.addColumns(NoticeOrderDetailVO.class);
	 ...
	 * 기    능 : 공급차량조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveVhcno")
	public void retrieveVhcno(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<NoticeVhcnoVO> list = deliveryNoticeMdsService.retrieveVhcno(spyplNaTrplC);
	 ...
	 * 기    능 : R1 사업장코드 조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveR1NaBzplC")
	public void retrieveR1NaBzplC(HttpServletRequest request, HttpServletResponse response) {
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<Map<String, Object>> list = deliveryNoticeMdsService.retrieveR1NaBzplC();
	 ...
	 * 기    능 	: 저장
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/saveDeliveryNoticeOrder")
	public void saveDeliveryNoticeOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String slpNo = requestHelper.getParameter("slp_no"); // 장비발주번호. 무발주형태로 작성하기 때문에 DataSet의 값은 지우고, 업데이트를 위한 PK조회용으로 파라메터를 받음
			...
			if(dsM != null && dsM.getRowCount() > 0){
				...
				if (	StringUtils.equals("8808983322353", dsM.getString(0, "ODRPL_NA_TRPL_C")) == false	// 농협안성저온물류센터(식자재)
					 ...
					 && StringUtils.equals("8808983322353", dsM.getString(0, "ODRPL_NA_TRPL_C")) == false	// 농협삼동저온물류센터(소매)
					 && StringUtils.equals("8808983325958", dsM.getString(0, "ODRPL_NA_TRPL_C")) == false	// 농협삼동저온물류센터(식자재)
					) {
					for (Map<String, Object> map : deliveryNoticeMdsService.retrieveR1NaBzplC()) {
				...
				if (isR1 == true && "1".equals(dsM.getString(0, "DDLY_YN")) == false && isDstr == false) {
					if (StringUtils.isEmpty(odrSlpno) || "undefined".equals(odrSlpno)) {	// 1st. R1 이고, 무발주(수주번호가 없음)인 경우
						...
						if (StringUtils.isEmpty(naDvyPlashSlpno)) {		// 배송예정서 등록인 경우 발주상태가 배송예정(03)상태인지 체크한다.
							...
							odrDList = deliveryNoticeMdsService.getOrderProduct(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpno, dvyPlaDt, spyplNaTrplC, naOdrStsDsc);
							...
							if (isSave == true && StringUtils.isEmpty(dvyRqrDt)) {	// 배송요청일이 null 인 경우 조회한다.
								dvyRqrDt = deliveryNoticeMdsService.getDeliveryRequestDate(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpno);
							}
						...
						if (isSave) {
							for (int i = 0; i < dsD.getRowCount(); i++) {
								...
								if (deliveryNoticeMdsService.checkAlcoholProduct(naWrsC) == false) {
									boolean odrYn = false;
					...
					if (StringUtils.isEmpty(naDvyPlashSlpno) == false) {	// 배송예정서 참조인 경우
						String DVY_PLA_DT = deliveryNoticeMdsService.getDvyPlaDt(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno, dvyPlaDt);
...
//				}
				...
				if (isSave) {
					...
					for(int i = 0; i < dsList.size(); i++){
						...
						VOConvertHelper voConverHelper 	= VOConvertHelper.newInstance(requestHelper, dsM, dsList.get(i), deliveryNoticeMdsService, seqKeyService, rogoInfCrtDsc, editType);
						
						List<DeliveryNoticeOrderVO> convertVO = voConverHelper.getDeliveryNoticeOrderVoList();
						deliveryNoticeMdsService.saveDeliveryNoticeOrder(editType, rogoInfCrtDsc, noVoList.get(0), convertVO);
						...
						if(StringUtils.equals("N", dsM.getString(0, "TEMP_YN"))){
							try {
								...
								if (StringUtils.isNotBlank(slpNo)) {
									// 배송예정서번호 업데이트
									deliveryNoticeMdsService.updateMdsOrderSlpno(slpNo, voConverHelper.getNaDvyPlashSlpno());
								}
	 ...
	 * 기    능 : 배송예정서 번호로 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getDeliveryNoticeOrder")
	public void getDeliveryNoticeOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			NoticeOrderVO vo = deliveryNoticeMdsService.getDeliveryNoticeOrder(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno, mbcoType, truAgcyNaTrplC);
	 ...
	 * 기    능 : 배송예정서 번호로 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getDeliveryNoticeOrderDetail")
	public void getDeliveryNoticeOrderDetail(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//배송예정서 상세
			List<NoticeOrderDetailVO> list = deliveryNoticeMdsService.getDeliveryNoticeOrderDetail(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno, dvyPlaDt, ddlyYn, cserCtrTpc, mbcoType, truAgcyNaTrplC);
			if(list != null){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
				dsHelper.addColumns(NoticeOrderDetailVO.class);
	...
	/* lsh
	 * 화면ID : VAN_DS_SC_0313, VAN_DS_SC_0310
	 * 화면명 : 배송예정서 장비등록
	 * 기    능 : 거래명세서출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryNoticeOrder")
	public RestResult printDeliveryNoticeOrder(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeMdsService.printDeliveryNoticeOrder(naDvyPlashSlpno, spyplNaTrplC, dvyaaNaTrplC, mbcoType, truAgcyNaTrplC, ozType);
	 ...
	 * 기  능 : 덤상품 거래명세서 출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryNoticeOrderDum")
	public RestResult printDeliveryNoticeOrderDum(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeMdsService.printDeliveryNoticeOrderDum(naDvyPlashSlpno, spyplNaTrplC, dvyaaNaTrplC, mbcoType, truAgcyNaTrplC, ozType);
	 ...
	 * 기    능 : 거래명세서출력
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryNoticeOrderList")
	public RestResult printDeliveryNoticeOrderList(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			for(int i = 0; i < naDvyPlashSlpnoArr.length; i++){
				...
				List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeMdsService.printDeliveryNoticeOrder(naDvyPlashSlpnoArr[i], spyplNaTrplC, dvyaaNaTrplC, mbcoType, truAgcyNaTrplC, ozType);
	 ...
	 * 화면명 : 발주번호_배송요청일 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveOrdersPopUp")
	public void retrieveOrdersPopUp(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<DeliveryNoticeOrderPopUpVO> list = deliveryNoticeMdsService.retrieveOrdersPopUp(odrSlpno, rvoplNaTrplC, odrplNaTrplC, dvyRqrDtFrom, dvyRqrDtTo, ddlyYn, odrplNaTrplN);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
	 ...
	 * 화면명 : 배송예정 거래처별 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveDeliveryTradingPatnersM")
	public void retrieveDeliveryTradingPatnersM(HttpServletRequest request, HttpServletResponse response) {
		...
		int totalRowCount = deliveryNoticeMdsService.getDeliveryTradingPatnersMCount(srhVo);
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<DeliveryTradingPatnersMVO> list = deliveryNoticeMdsService.retrieveDeliveryTradingPatnersM(srhVo);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
			...
			//합계
			Map<String, Object> sumMap = deliveryNoticeMdsService.getDeliveryTradingPatnersMSum(srhVo);
	 ...
	 * 기    능 : 거래처별/상품별 Print
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryTradingPatners01")
	public RestResult printDeliveryTradingPatners01(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			//Data Set
			List<List<?>> all_list = new ArrayList<List<?>>();
			List<DeliveryTradingPatnersMVO> mList = deliveryNoticeMdsService.printDeliveryTradingPatnersM(srhVo);
	 ...
	 * 기    능 : 거래처별 Print
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryTradingPatners02")
	 ...
	 * 화면명 : 배송예정 거래처별 조회 상세
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveDeliveryTradingPatnersD")
	public void retrieveDeliveryTradingPatnersD(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			int totalRowCount = deliveryNoticeMdsService.getDeliveryTradingPatnersDCount(spypl_na_trpl_c, dvyaa_na_trpl_c, na_dvy_plash_slpno, slpDt, naSlpno, mbcoType, truAgcyNaTrplC);
			//Paging
			...
			PlatformData out_PlatformData = new PlatformData();
			List<DeliveryTradingPatnersDVO> list = deliveryNoticeMdsService.retrieveDeliveryTradingPatnersD(spypl_na_trpl_c, dvyaa_na_trpl_c, na_dvy_plash_slpno, start, end, sort, slpDt, naSlpno, mbcoType, truAgcyNaTrplC);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDeliveryTradingPatners")
	public void downloadExcelDeliveryTradingPatners(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> list = deliveryNoticeMdsService.downloadExcelDeliveryTradingPatners(srhVo);
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDeliveryTradingPatnersM")
	public void downloadExcelDeliveryTradingPatnersM(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<DeliveryTradingPatnersMVO> list = deliveryNoticeMdsService.downloadExcelDeliveryTradingPatnersM(srhVo);
			String[] header = new String[]{
					"수주처",
					"수주처명",
					"배송지",
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDeliveryTradingPatnersD")
	public void downloadExcelDeliveryTradingPatnersD(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<DeliveryTradingPatnersDVO> list = deliveryNoticeMdsService.retrieveDeliveryTradingPatnersD(spypl_na_trpl_c, dvyaa_na_trpl_c, na_dvy_plash_slpno, 1, 150, sort, slpDt, naSlpno, mbcoType, truAgcyNaTrplC);
			String[] header = new String[]{
					"경제통합상품코드",
					"경제통합상품명",
					"발주전표일련번호",
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDetailDeliveryTradingPatners")
	public void downloadExcelDetailDeliveryTradingPatners(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> list = deliveryNoticeMdsService.downloadExcelDetailDeliveryTradingPatners(srhVo);
			String[] header = new String[]{
					"00공급처경제통합거래처코드",
					"01공급처경제통합팀코드",
					"02수주처경제통합거래처코드",
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDeliveryTradingPatnersE")
	public void downloadExcelDeliveryTradingPatnersE(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<DeliveryTradingPatnersExcelDetailVO> list = deliveryNoticeMdsService.downloadExcelDeliveryTradingPatnersE(srhVo);
			String[] header = new String[]{
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveDeliverySkusM")
	public void retrieveDeliverySkusM(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int totalRowCount = deliveryNoticeMdsService.getDeliverySkusMCount(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR);
			//Paging
			PageHelper pageHelper = PageHelper.newInstance(requestHelper.getDataSet("ds_pageVO01"), totalRowCount, "ds_pageVO01");
			int start = pageHelper.getStart();
			int end  = pageHelper.getEnd();
			PlatformData out_PlatformData = new PlatformData();
			List<DeliverySkusMVO> list = deliveryNoticeMdsService.retrieveDeliverySkusM(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, start, end, mbcoType, truAgcyNaTrplC);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveDeliverySkusD")
...
//			String spyplType = requestHelper.getParameter("spypl_type");
			...
			int totalRowCount = deliveryNoticeMdsService.getDeliverySkusDCount(naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, mbcoType, truAgcyNaTrplC);
			//Paging
			PageHelper pageHelper = PageHelper.newInstance(requestHelper.getDataSet("ds_pageVO02"), totalRowCount, "ds_pageVO02");
			int start = pageHelper.getStart();
			int end  = pageHelper.getEnd();
			PlatformData out_PlatformData = new PlatformData();
			List<DeliverySkusDVO> list = deliveryNoticeMdsService.retrieveDeliverySkusD(naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, start, end, mbcoType, truAgcyNaTrplC);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDeliverySkus")
	public void downloadExcelDeliverySkus(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> list = deliveryNoticeMdsService.downloadExcelDeliverySkus(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, mbcoType, truAgcyNaTrplC);
			String[] header = new String[]{
					"00경제통합상품코드",
					"01상품명",
					"02상품규격명",
	 ...
	 * 화면명 : 배송예정 상품별 조회 출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliverySkus")
	public RestResult printDeliverySkus(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			List<DeliverySkusMVO> masterVOList = deliveryNoticeMdsService.retrieveDeliverySkusM(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, -1, -1, mbcoType, truAgcyNaTrplC);
			...
			if(masterVOList != null && masterVOList.size() > 0){
				...
				for(DeliverySkusMVO vo : masterVOList){
					...
					List<DeliverySkusDVO> detailVOList = deliveryNoticeMdsService.retrieveDeliverySkusD(vo.getNA_WRS_C(), dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, -1, -1, mbcoType, truAgcyNaTrplC);
	 ...
	 * 화면명 : 배송예정 결과조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/retrieveDeliveryResult")
	public void retrieveDeliveryResult(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int totalRowCount = deliveryNoticeMdsService.getDeliveryResultCount(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR, naDvyPlashSlpno, odrSlpno, mbcoType, truAgcyNaTrplC);
			//Paging
			PageHelper pageHelper = PageHelper.newInstance(requestHelper.getDataSet("ds_pageVO"), totalRowCount, "ds_pageVO");
			int start = pageHelper.getStart();
			int end  = pageHelper.getEnd();
			PlatformData out_PlatformData = new PlatformData();
			List<Map<String, Object>> list = deliveryNoticeMdsService.retrieveDeliveryResult(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR, naDvyPlashSlpno, odrSlpno, start, end, mbcoType, truAgcyNaTrplC);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
	...
	@RequestMapping(method=RequestMethod.POST, value = "/rest/escel/mds/delivery/downloadExcelDeliveryResult")
	public void downloadExcelDeliveryResult(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> list = deliveryNoticeMdsService.retrieveDeliveryResult(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR, naDvyPlashSlpno, odrSlpno, -1, -1, mbcoType, truAgcyNaTrplC);
			String[] header = new String[]{
					"순번",
					"발주처",
					"발주처명",
	...
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/mds/delivery/downloadExcelDeliveryDetailResult")
	public void downloadExcelDeliveryDetailResult(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<Map<String, Object>> list = deliveryNoticeMdsService.sql_retrieveDeliveryDetailResult(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR, naDvyPlashSlpno, odrSlpno, -1, -1, mbcoType, truAgcyNaTrplC);
			String[] header = new String[]{
					"순번",
					"발주처",
					"발주처명",
	 ...
	 * 기    능 : 거래명세서출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryNoticeOrders")
	public RestResult printDeliveryNoticeOrders(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			List<DeliveryTradingPatnersMVO> dvyMasterList = deliveryNoticeMdsService.printDeliveryTradingPatnersJustM(srhVo);
			if(dvyMasterList != null && dvyMasterList.size() > 0) {
				int keyIndex = 0;
				for(DeliveryTradingPatnersMVO mvo : dvyMasterList) {
					List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeMdsService.printDeliveryNoticeOrder(mvo.getNA_DVY_PLASH_SLPNO(), mvo.getSPYPL_NA_TRPL_C(),
	 ...
	 * 기    능 : 덤상품 거래명세서출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/mds/delivery/printDeliveryNoticeOrdersDum")
	public RestResult printDeliveryNoticeOrdersDum(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			if(dvyMasterList != null && dvyMasterList.size() > 0) {
				int keyIndex = 0;
				for(DeliveryTradingPatnersMVO mvo : dvyMasterList) {
					List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeMdsService.printDeliveryNoticeOrderDum(mvo.getNA_DVY_PLASH_SLPNO(), mvo.getSPYPL_NA_TRPL_C(),
							mvo.getDVYAA_NA_TRPL_C(), mbcoType, truAgcyNaTrplC, ozType);
					if(list != null && list.size() > 0){
						Map<String,Object> _master = new LinkedHashMap<String, Object>();
						//Master Set
	 ...
	 * 기    능 : 영업가능 여부
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getBizDtYn")
	public void getBizDtYn(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			String bizDtYn = deliveryNoticeMdsService.getBizDtYn(odrplNaTrplC, searchDate);
			String bizDt = deliveryNoticeMdsService.getBizDt(trplO, teamO, trplR, teamR, searchDate);
	 ...
	 * 기    능 : 영업가능일
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getBizDt")
	 ...
	 * 기    능 : 상품 바이어 조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/getBuyer")
	public void getBuyer(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			Map<String, Object> resultMap = deliveryNoticeMdsService.getBuyer(naWrsN, naWrsC, naWrsLclc, naWrsMclc, naWrsSclc, naWrsDtcfC);
	 ...
	 * 화면명 : 배송예정서 일반 취소
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/deliveryNoticeCancelProc")
...
//		String truAgcyNaTrplC = requestHelper.getGlnCode();	// 위탁대리점 코드
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			/*int count = */deliveryNoticeMdsService.deliveryNoticeCancelProc(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno, userID);
	...
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/isOKdlvpnsht")
	 ...
	 * 빌링 서비스
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/mds/delivery/billangMarkUp")
