...
public class LiveStockVOConvertHelper {
	...
	public static LiveStockVOConvertHelper newInstance(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, DeliveryNoticeLiveStockService deliveryNoticeLiveStockService, SequenceKeyService seqKeyService, String rogoInfCrtDsc, String editType){
		return new LiveStockVOConvertHelper(requestHelper, dsM, dsD, deliveryNoticeLiveStockService, seqKeyService, rogoInfCrtDsc, editType);
	}
	
	public static LiveStockVOConvertHelper newInstance(DataSet dsM, DataSet dsD){
		return new LiveStockVOConvertHelper(dsM, dsD);
	}
	
	public static LiveStockVOConvertHelper newInstance(NoticeOrderTempFileMasterVO tmpMvo, String naDvyPlashSlpno){
		return new LiveStockVOConvertHelper(tmpMvo, naDvyPlashSlpno);
	}
	
	//배송예정서 일괄저장
	public LiveStockVOConvertHelper(NoticeOrderTempFileMasterVO tmpMvo, String naDvyPlashSlpno){
		this.tmpMvo = tmpMvo;
		this.naDvyPlashSlpno = naDvyPlashSlpno;
	...
	//배송예정서 일반등록
	public LiveStockVOConvertHelper(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, DeliveryNoticeLiveStockService deliveryNoticeLiveStockService, SequenceKeyService seqKeyService, String rogoInfCrtDsc, String editType) {
		this.requestHelper = requestHelper;
		this.dsM = dsM;
		this.dsD = dsD;
		this.seqKeyService = seqKeyService;
		this.deliveryNoticeLiveStockService = deliveryNoticeLiveStockService;
		makeSaveNoticeOrderVO(requestHelper, dsM, dsD, rogoInfCrtDsc, editType);
	}
	public LiveStockVOConvertHelper(DataSet dsM, DataSet dsD) {
		this.dsM = dsM;
		this.dsD = dsD;
	}
	...
	public void makeSaveNoticeOrderVO(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, String rogoInfCrtDsc, String editType){
		...
		final String DVY_PLASH_KDC_07 = "07";//축산(직송)발주 (20210713)
		...
		try{
			...
			} else if (StringUtils.equals(DVY_PLASH_KDC_07, rogoInfCrtDsc)) {
				dvyPlashKdc = DVY_PLASH_KDC_07;
			} else {
				...
				if (StringUtils.equals(tempYn, "N")) { // 완료
					if (StringUtils.equals(DVY_PLASH_KDC_02, rogoInfCrtDsc)) { // 02 : 덤발주, 01 : 일반발주
						...
						for (int i = 0; i < 2; i++) {
							...
							naDvyPlashSlpno = deliveryNoticeLiveStockService.getNaDvyPlashSlpno(trplC, teamC, dvyPlaDt, ddlyYn);
	...
	private DeliveryNoticeOrderVO updateDeliveryNoticeOrder(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD){
		...
		for(int i = 0; i < dsD.getRowCount(); i++){
			...
			if(StringUtils.equals(dsD.getString(i, "DEL_YN"), "1") == true){
				...
				*/
				...
				double dvyVat = 0;
				if(StringUtils.equals("1", nodVo.getTXT_DSC())){
					dvyVat = Double.valueOf(deliveryNoticeLiveStockService.getDvyVat(nodVo.getDVY_PLA_QT(), nodVo.getDVY_WRS_UPR(), nodVo.getTXT_DSC()));
				}
				...
				dnodVo.setDQPD_RSNC(nodVo.getDQPD_RSNC());//결품사유
				...
				dnodVo.setBOX_QT(nodVo.getBOX_QT());//박스수량
				dnodVo.setINDV_ID_NO(nodVo.getINDV_ID_NO());//개체식별번호
		...
		dnVo.setSRA_WRS_DSC(mvo.getSRA_WRS_DSC());
		dnVo.setNA_PAK_DSC(mvo.getNA_PAK_DSC());
		dnVo.setSFP_BOX_DSC(mvo.getSFP_BOX_DSC());
		dnVo.setSRA_WK_DSC(mvo.getSRA_WK_DSC());
		dnVo.setDVY_WT(totalDvyPlaQt);
	...
	private DeliveryNoticeOrderVO insertDeliveryNoticeOrder(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, String naDvyPlashSlpno, String dvyPlashKdc, String editType) throws Exception{
		try{
			...
			dnVo.setDVYAA_NA_TRPL_C(mvo.getDVYAA_NA_TRPL_C()); //배송처거래처코드(물건이 도착하는 곳)
			dnVo.setDVYAA_NA_TEAM_C(mvo.getDVYAA_NA_TEAM_C());//배송처 팀코드
			dnVo.setNA_DVY_PLASH_SLPNO(naDvyPlashSlpno); //배송예정서 전표번호
