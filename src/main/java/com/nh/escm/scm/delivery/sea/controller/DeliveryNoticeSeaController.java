package com.nh.escm.scm.delivery.sea.controller;
...
import com.nh.escm.scm.delivery.sea.helper.ConvertHelper;
import com.nh.escm.scm.delivery.sea.helper.VOConvertHelper;
...
import com.nh.escm.scm.delivery.sea.vo.NoticeOrderTempFileVO;
 ...
 */
@RestController
public class DeliveryNoticeSeaController extends AbstractController {
	...
	@Autowired
	private DeliveryNoticeSeaService deliveryNoticeSeaService;
	 ...
	 * 기    능 : 영업가능일
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveTC")
	public void retrieveTC(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			List<DeliveryTcVO> list = deliveryNoticeSeaService.retrieveTC(trplC);
	 ...
	 * 기    능 : 발주 정보 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getOrder")
	public void getOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			NoticeOrderVO vo = deliveryNoticeSeaService.getOrder(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpNo);
			...
			//발주상세정보
			List<NoticeOrderDetailVO> list = deliveryNoticeSeaService.getOrderDetail(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpNo);
			if(list != null){
	 ...
	 * 기    능 : 발주 정보 상세조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getOrderDetail")
	public void getOrderDetail(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			//발주상세정보
			List<NoticeOrderDetailVO> list = deliveryNoticeSeaService.getOrderDetail(odrplNaTrplC, odrplNaTeamC, odrDt, odrSlpNo, dvyPlaDt);
			if(list != null){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
				dsHelper.addColumns(NoticeOrderDetailVO.class);
	 ...
	 * 기    능 : 결품사유코드 조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveShortageReason")
	public void retrieveShortageReason(HttpServletRequest request, HttpServletResponse response) {
		PlatFormRequestHelper requestHelper = PlatFormRequestHelper.newInstance(request);

		String code = requestHelper.getParameter("SIMP_TPC");
		
		try {
			PlatformData platformData = new PlatformData();
			List<Map<String, Object>> list = deliveryNoticeSeaService.retrieveShortageReason(code);
			
			if (list.isEmpty() == false) {
				DataSetHelper dsHelper = new DataSetHelper("ds_dqpd_rsnc_c");
	 ...
	 * 기    능 : 수산부 결품내역 조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/delivery/sea/retrieveShortageReasonList")
	public void retrieveShortageReasonList(HttpServletRequest request, HttpServletResponse response) {
		PlatFormRequestHelper requestHelper = PlatFormRequestHelper.newInstance(request);
		
		String spyplNaTrplC = requestHelper.getParameter("spyplNaTrplC"); //공급처코드
		String dvyaaNaTrplC = requestHelper.getParameter("dvyaaNaTrplC"); //배송처코드
		String dvyaaNaTeamC = requestHelper.getParameter("dvyaaNaTeamC"); //배송처팀코드
		String dvyFrom = requestHelper.getParameter("dvyFrom");
		String dvyTo = requestHelper.getParameter("dvyTo");
		
		String[] spyplNaTrplCArr = requestHelper.getTrplCodeToArr(spyplNaTrplC, requestHelper);
		
		try {
			PlatformData platformData = new PlatformData();
			List<Map<String, Object>> list = deliveryNoticeSeaService.retrieveShortageReasonList(spyplNaTrplCArr, dvyaaNaTrplC, dvyaaNaTeamC, dvyFrom, dvyTo);
			
			if (list.isEmpty() == false) {
				DataSetHelper dsHelper = new DataSetHelper("ds_data");
				dsHelper.addColumns(list.get(0).keySet());
	 ...
	 * 기    능 : 공급차량조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveVhcno")
	public void retrieveVhcno(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<NoticeVhcnoVO> list = deliveryNoticeSeaService.retrieveVhcno(spyplNaTrplC);
	 ...
	 * 기    능 	: 저장
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/saveDeliveryNoticeOrder")
...
//			boolean isDstr = false; //저온여부
			...
			if(dsM != null && dsM.getRowCount() > 0){
				...
				// 수산부의 경우 배송예정서 작성시 결품이 있는데 결품사유를 미등록하면 결품패널티를 부과한다.
				String seaOdrpl = "8808983324388|8808983326634|8808983326641|8808983410142";				
				for (int i = 0; i < dsD.getRowCount(); i++) {
					...
					if (odrQt > dvyPlaQt) {														// 수주량보다 배송예정량이 적은 경우
						String dqpdRsnc = dsD.getString(i, "DQPD_RSNC");
						String dqpdRsnCntn = dsD.getString(i, "DQPD_RSN_CNTN");
						
						if (StringUtils.isEmpty(dqpdRsnc)) {									// 결품사유코드 check
							if (seaOdrpl.indexOf(dsM.getString(0, "ODRPL_NA_TRPL_C")) != -1) {
								flag = false;
								sendData(response, new PlatformData(), -97, "결품사유코드를 입력해야 합니다.");
							} else {
								// 발주처가 수산부가 아닐 경우 결품사유를 필수값으로 받지 않음
								dsD.set(i, "DQPD_RSNC", "");
								dsD.set(i, "DQPD_RSN_CNTN", "");
							}
						} else {
							if ("99".equals(dqpdRsnc) && StringUtils.isEmpty(dqpdRsnCntn)) {	// 결품사유코드가 99일 경우 결품사유 check
								flag = false;
								sendData(response, new PlatformData(), -98, "결품사유를 입력해야 합니다.");
							}
						}
					} else {	// 수주량보다 배송예정량이 같거나 많은 경우 결품사유코드와 결품사유를 초기화 해준다.
						dsD.set(i, "DQPD_RSNC", "");
						dsD.set(i, "DQPD_RSN_CNTN", "");
					}
				}
...
//				}
				...
				if (flag) {
					...
					for(int i = 0; i < dsList.size(); i++){
						...
						VOConvertHelper voConverHelper 	= VOConvertHelper.newInstance(requestHelper, dsM, dsList.get(i), deliveryNoticeSeaService, seqKeyService, rogoInfCrtDsc, editType);
						
						List<DeliveryNoticeOrderVO> convertVO = voConverHelper.getDeliveryNoticeOrderVoList();
						deliveryNoticeSeaService.saveDeliveryNoticeOrder(editType, rogoInfCrtDsc, noVoList.get(0), convertVO);
	 ...
	 * 기    능 : 배송예정서 번호로 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getDeliveryNoticeOrder")
	public void getDeliveryNoticeOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			NoticeOrderVO vo = deliveryNoticeSeaService.getDeliveryNoticeOrder(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno);
	 ...
	 * 기    능 : 배송예정서 번호로 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getDeliveryNoticeOrderDetail")
	public void getDeliveryNoticeOrderDetail(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//배송예정서 상세
			List<NoticeOrderDetailVO> list = deliveryNoticeSeaService.getDeliveryNoticeOrderDetail(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno, dvyPlaDt, ddlyYn, cserCtrTpc);
			if(list != null){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
				dsHelper.addColumns(NoticeOrderDetailVO.class);
	...
	/* lsh
	 * 화면ID : VAN_DS_SC_0302, VAN_DS_SC_0310
	 * 화면명 : 배송예정서 일반등록
	 * 기    능 : 거래명세서출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/delivery/sea/printDeliveryNoticeOrder")
	public RestResult printDeliveryNoticeOrder(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			if(true){
				List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeSeaService.printDeliveryNoticeOrder(naDvyPlashSlpno, spyplNaTrplC, dvyaaNaTrplC);
				...
				*/
				...
				if(list != null && list.size() > 0){
					...
					String sBizNo = "";
					sBizNo = vo.getSPYPL_BIZ_NUMBER().substring(0,3) + "-" + vo.getSPYPL_BIZ_NUMBER().substring(3,5) + "-" + vo.getSPYPL_BIZ_NUMBER().substring(5);
					String dBizNo = "";
					dBizNo = vo.getDVYAA_BIZ_NUMBER().substring(0,3) + "-" + vo.getDVYAA_BIZ_NUMBER().substring(3,5) + "-" + vo.getDVYAA_BIZ_NUMBER().substring(5);
					...
					_master.put("master03", sBizNo); //사업자번호
					...
					*/				
					...
					for(DeliveryNoticeOrderPrintVO item : list){
						...
						if(Integer.parseInt(item.getODR_QT()) != 0){
							unpaidQt = Double.parseDouble(item.getODR_QT()) - Double.parseDouble(item.getDVY_PLA_QT());
							unpaidAmt = (long)(unpaidQt * Double.parseDouble(item.getDVY_WRS_UPR()));
						}
						...
						_detail.put("NUM_detail06", item.getODR_QT());//발주수량
						_detail.put("NUM_detail07", item.getDVY_PLA_QT());//낱개수량
						...
						_detail.put("NUM_detail11", item.getSPYPL_AM());//공급금액
						_detail.put("NUM_detail12", unpaidQt);//미납수량
						_detail.put("NUM_detail13", unpaidAmt);//미납금액
						_detail.put("detail14", item.getPDAA_NM());//원산지
	 ...
	 * 기    능 : Excel File 조회
	 */	
	@RequestMapping(method=RequestMethod.POST,value="/rest/scm/order/sea/downloadDeliveryNoticeOrder")
		...
		try {
			...
			if(list.size() > 100){
				...
				if(reDs.getRowCount() > 0){
					...
					deliveryNoticeSeaService.insertNoticeOrderMTmp(tmpMList);
	 ...
	 * 기    능 : 저장
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/insertDeliveryNoticeOrder")
		...
		try {
			...
			if(inDs.getRowCount() == 0){
				...
				for(int i = 0; i < tmpSeqDs.getRowCount(); i++){
					...
					List<NoticeOrderTempFileMasterVO> mvoList = deliveryNoticeSeaService.retreiveNoticeOrderMTmp(tempSeq);
					...
					for(NoticeOrderTempFileMasterVO mvo : mvoList){
						...
						String naDvyPlashSlpno = deliveryNoticeSeaService.getNaDvyPlashSlpno(mvo.getRVOPL_NA_TRPL_C(), mvo.getRVOPL_NA_TEAM_C(), mvo.getDVY_PLA_DT(), ddlyYn);
						...
						List<DeliveryTrnrelVO> trnrelList = deliveryNoticeSeaService.retrieveTrnrel(odrplNaTrplC, odrplNaTeamC, splrNaTrplC, splrNaTeamC, cserCtrTpc, spyTpc, mngtNaTrplC, mngtNaTeamC, mngtNaTrTpc, naWrsLclc, naWrsMclc);
					...
					if(mvoList.size() == list.size()){
						deliveryNoticeSeaService.insertDeliveryNoticeOrder(list);
					}
				}
	 ...
	 * 화면명 : 발주번호_배송요청일 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveOrdersPopUp")
	public void retrieveOrdersPopUp(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<DeliveryNoticeOrderPopUpVO> list = deliveryNoticeSeaService.retrieveOrdersPopUp(odrSlpno, rvoplNaTrplC, odrplNaTrplC, dvyRqrDtFrom, dvyRqrDtTo, ddlyYn, odrplNaTrplN);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
	 ...
	 * 화면명 : 배송예정 거래처별 조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveDeliveryTradingPatnersM")
	public void retrieveDeliveryTradingPatnersM(HttpServletRequest request, HttpServletResponse response) {
		...
		int totalRowCount = deliveryNoticeSeaService.getDeliveryTradingPatnersMCount(srhVo);
		//Paging
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			List<DeliveryTradingPatnersMVO> list = deliveryNoticeSeaService.retrieveDeliveryTradingPatnersM(srhVo);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
			...
			//합계
			Map<String, Object> sumMap = deliveryNoticeSeaService.getDeliveryTradingPatnersMSum(srhVo);
	 ...
	 * 기    능 : 거래처별/상품별 Print
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/scm/delivery/sea/printDeliveryTradingPatners01")
	public RestResult printDeliveryTradingPatners01(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			//Data Set
			List<List<?>> all_list = new ArrayList<List<?>>();
			List<DeliveryTradingPatnersMVO> mList = deliveryNoticeSeaService.printDeliveryTradingPatnersM(srhVo);
	 ...
	 * 기    능 : 거래처별 Print
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/scm/delivery/sea/printDeliveryTradingPatners02")
	 ...
	 * 화면명 : 배송예정 거래처별 조회 상세
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveDeliveryTradingPatnersD")
	public void retrieveDeliveryTradingPatnersD(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			int totalRowCount = deliveryNoticeSeaService.getDeliveryTradingPatnersDCount(spypl_na_trpl_c, dvyaa_na_trpl_c, na_dvy_plash_slpno, slpDt, naSlpno);
			//Paging
			...
			PlatformData out_PlatformData = new PlatformData();
			List<DeliveryTradingPatnersDVO> list = deliveryNoticeSeaService.retrieveDeliveryTradingPatnersD(spypl_na_trpl_c, dvyaa_na_trpl_c, na_dvy_plash_slpno, start, end, sort, slpDt, naSlpno);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/scm/delivery/sea/downloadExcelDeliveryTradingPatners")
	public void downloadExcelDeliveryTradingPatners(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			deliveryNoticeSeaService.downloadExcelDeliveryTradingPatners(srhVo, handler);
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/scm/delivery/sea/downloadExcelDeliveryTradingPatnersM")
	public void downloadExcelDeliveryTradingPatnersM(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			deliveryNoticeSeaService.downloadExcelDeliveryTradingPatnersM(srhVo, handler);
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/scm/delivery/sea/downloadExcelDetailDeliveryTradingPatners")
	public void downloadExcelDetailDeliveryTradingPatners(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			deliveryNoticeSeaService.downloadExcelDetailDeliveryTradingPatners(srhVo, handler);
			...
			List<Map<String, Object>> billingList = deliveryNoticeSeaService.downloadExcelDetailDeliveryTradingPatnersBilling(srhVo);
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/scm/delivery/sea/downloadExcelDeliveryTradingPatnersE")
	public void downloadExcelDeliveryTradingPatnersE(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			deliveryNoticeSeaService.downloadExcelDeliveryTradingPatnersE(srhVo, handler);
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveDeliverySkusM")
	public void retrieveDeliverySkusM(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int totalRowCount = deliveryNoticeSeaService.getDeliverySkusMCount(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR);
			//Paging
			PageHelper pageHelper = PageHelper.newInstance(requestHelper.getDataSet("ds_pageVO01"), totalRowCount, "ds_pageVO01");
			int start = pageHelper.getStart();
			int end  = pageHelper.getEnd();
			PlatformData out_PlatformData = new PlatformData();
			List<DeliverySkusMVO> list = deliveryNoticeSeaService.retrieveDeliverySkusM(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, start, end);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveDeliverySkusD")
	public void retrieveDeliverySkusD(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int totalRowCount = deliveryNoticeSeaService.getDeliverySkusDCount(naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR);
			//Paging
			PageHelper pageHelper = PageHelper.newInstance(requestHelper.getDataSet("ds_pageVO02"), totalRowCount, "ds_pageVO02");
			int start = pageHelper.getStart();
			int end  = pageHelper.getEnd();
			PlatformData out_PlatformData = new PlatformData();
			List<DeliverySkusDVO> list = deliveryNoticeSeaService.retrieveDeliverySkusD(naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, start, end);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataD");
	 ...
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/scm/delivery/sea/downloadExcelDeliverySkus")
	public void downloadExcelDeliverySkus(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			deliveryNoticeSeaService.downloadExcelDeliverySkus(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, handler);
	 ...
	 * 화면명 : 배송예정 상품별 조회 출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/scm/delivery/sea/printDeliverySkus")
	public RestResult printDeliverySkus(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			List<DeliverySkusMVO> masterVOList = deliveryNoticeSeaService.retrieveDeliverySkusM(wrsGubun, naWrsC, dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, -1, -1);
			...
			if(masterVOList != null && masterVOList.size() > 0){
				...
				for(DeliverySkusMVO vo : masterVOList){
					...
					List<DeliverySkusDVO> detailVOList = deliveryNoticeSeaService.retrieveDeliverySkusD(vo.getNA_WRS_C(), dvyPlaDtFr, dvyPlaDtTo, TRPL_C_ARR, -1, -1);
	 ...
	 * 화면명 : 배송예정 결과조회
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/retrieveDeliveryResult")
	public void retrieveDeliveryResult(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			int totalRowCount = deliveryNoticeSeaService.getDeliveryResultCount(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR);
			//Paging
			PageHelper pageHelper = PageHelper.newInstance(requestHelper.getDataSet("ds_pageVO"), totalRowCount, "ds_pageVO");
			int start = pageHelper.getStart();
			int end  = pageHelper.getEnd();
			PlatformData out_PlatformData = new PlatformData();
			List<Map<String, Object>> list = deliveryNoticeSeaService.retrieveDeliveryResult(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR, start, end);
			if(list != null && list.size() > 0){
				DataSetHelper dsHelper = new DataSetHelper("ds_dataM");
	...
	@RequestMapping(method=RequestMethod.POST, value = "/rest/excel/scm/delivery/sea/downloadExcelDeliveryResult")
	public void downloadExcelDeliveryResult(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			deliveryNoticeSeaService.downloadExcelDeliveryResult(dvyPlaDtFr, dvyPlaDtTo, odrplNaTrplC, ddlyYn, resCode, TRPL_C_ARR, -1, -1, handler);
	 ...
	 * 기    능 : 거래명세서출력
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/oz/delivery/sea/printDeliveryNoticeOrders")
	public RestResult printDeliveryNoticeOrders(HttpServletRequest request, HttpServletResponse response,
		...
		try{
			...
			List<DeliveryTradingPatnersMVO> dvyMasterList = deliveryNoticeSeaService.printDeliveryTradingPatnersJustM(srhVo);
			
			if(dvyMasterList != null && dvyMasterList.size() > 0) {
				int keyIndex = 0;
				for(DeliveryTradingPatnersMVO mvo : dvyMasterList) {
					List<DeliveryNoticeOrderPrintVO> list = deliveryNoticeSeaService.printDeliveryNoticeOrder(mvo.getNA_DVY_PLASH_SLPNO(), mvo.getSPYPL_NA_TRPL_C(), mvo.getDVYAA_NA_TRPL_C());
					if(list != null && list.size() > 0){
						Map<String,Object> _master = new LinkedHashMap<String, Object>();
						//Master Set
						DeliveryNoticeOrderPrintVO vo = (DeliveryNoticeOrderPrintVO)list.get(0);
	 ...
	 * 기    능 : 영업가능 여부
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getBizDtYn")
	public void getBizDtYn(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			String bizDtYn = deliveryNoticeSeaService.getBizDtYn(odrplNaTrplC, searchDate);
			String bizDt = deliveryNoticeSeaService.getBizDt(trplO, teamO, trplR, teamR, searchDate);
			...
			if(StringUtils.equals("2", chkPart) && StringUtils.equals("0", bizDtYn)){
				sendData(response, new PlatformData(), -99, "msg.VAN_DS_SC_0302.dvy.pla_dt.holidy");
			}
	 ...
	 * 기    능 : 영업가능일
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getBizDt")
	 ...
	 * 기    능 : 상품 바이어 조회
	 */
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/getBuyer")
	public void getBuyer(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			Map<String, Object> resultMap = deliveryNoticeSeaService.getBuyer(naWrsN, naWrsC, naWrsLclc, naWrsMclc, naWrsSclc, naWrsDtcfC);
	 ...
	 * 화면명 : 배송예정서 수산 취소
	 */	
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/deliveryNoticeCancelProc")
	public void deliveryNoticeCancelProc(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			PlatformData out_PlatformData = new PlatformData();
			int count = deliveryNoticeSeaService.deliveryNoticeCancelProc(spyplNaTrplC, dvyaaNaTrplC, naDvyPlashSlpno, userID);
	...
	@RequestMapping(method=RequestMethod.POST, value = "/rest/scm/delivery/sea/isOKdlvpnsht")
