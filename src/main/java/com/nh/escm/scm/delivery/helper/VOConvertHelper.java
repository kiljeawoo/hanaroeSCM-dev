...
public class VOConvertHelper {
	...
	public static VOConvertHelper newInstance(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, DeliveryNoticeService deliveryNoticeService, SequenceKeyService seqKeyService, String rogoInfCrtDsc, String editType){
	...
	//배송예정서 일반등록
	public VOConvertHelper(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, DeliveryNoticeService deliveryNoticeService, SequenceKeyService seqKeyService, String rogoInfCrtDsc, String editType) {
		this.requestHelper = requestHelper;
		this.dsM = dsM;
		this.dsD = dsD;
		this.seqKeyService = seqKeyService;
	...
	private DeliveryNoticeOrderVO convertDeliveryNoticeOrderMVO(){
		...
		// 일괄등록의 혁신/PB직송 여부는 보류(일반으로만 등록하도록 고정)
...
//		 : DVY_PLASH_CRT_DSC = 002, PBDDLY_YN = 1
		...
		// 1) 가격군코드 체크
		String prgrC = dsM.getString(0, "PRGR_C");
		if ("70".equals(prgrC) || "80".equals(prgrC)) {
			mvo.setDVY_PLASH_CRT_DSC("002"); // 혁신점포
		}
		
		// 2) 수발주구분코드 체크
		String rogoInfCrtDsc = dsM.getString(0, "ROGO_INF_CRT_DSC");
		if (StringUtil.isNullCheck(mvo.getDVY_PLASH_CRT_DSC())) {
			if ("243".equals(rogoInfCrtDsc) || "244".equals(rogoInfCrtDsc) || "800".equals(rogoInfCrtDsc) || "801".equals(rogoInfCrtDsc) || "802".equals(rogoInfCrtDsc)) {
				mvo.setDVY_PLASH_CRT_DSC("002"); // 혁신점포
			} else {
				mvo.setDVY_PLASH_CRT_DSC("001"); // 일반
			}
		}
//		System.out.println("[U]배송예정서 작성 getDVY_PLASH_CRT_DSC : " + mvo.getDVY_PLASH_CRT_DSC());
//		System.out.println("[U]배송예정서 작성 getPBDDLY_YN : " + mvo.getPBDDLY_YN());
		
		// PB직송 여부 체크
		if ("245".equals(rogoInfCrtDsc)) {
			mvo.setPBDDLY_YN("1");
		}
		...
		dnVo.setPBDDLY_YN(mvo.getPBDDLY_YN()); //PB직송여부 : 직송=1, 일반=0
		dnVo.setDVY_PLASH_CRT_DSC(mvo.getDVY_PLASH_CRT_DSC());//배송예정서생성구분코드
		return dnVo;
	}
...
//			System.out.println("[I]배송예정서 작성 getDVY_PLASH_CRT_DSC : " + mvo.getDVY_PLASH_CRT_DSC());
//			System.out.println("[I]배송예정서 작성 getPBDDLY_YN : " + mvo.getPBDDLY_YN());
