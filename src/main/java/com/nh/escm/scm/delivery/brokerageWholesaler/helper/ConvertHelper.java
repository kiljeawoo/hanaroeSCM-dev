package com.nh.escm.scm.delivery.brokerageWholesaler.helper;
...
public class ConvertHelper {
	...
	DeliveryNoticeBrokerageWholesalerDAO deliveryNoticeDao = BeanUtil.getBean(DeliveryNoticeBrokerageWholesalerDAO.class);
	...
	public  List<DataSet> excelValidationChk(PlatFormRequestHelper requestHelper, DataSet inDs){
		...
		for(int i = 0; i < inDs.getRowCount(); i++){
			...
			List tmpList = orderOrdersDAO.getTrnrel(String.valueOf(inDs.getObject(i, 6)), String.valueOf(inDs.getObject(i, 7)), String.valueOf(inDs.getObject(i, 2))
					, String.valueOf(inDs.getObject(i, 3)), String.valueOf(inDs.getObject(i, 18)));
			
			if(tmpList != null && tmpList.size() > 0){
				tmpMmap = (Map)tmpList.get(0);
			}
			for(int j = 0; j < inDs.getColumnCount(); j++){
				...
				if(j == 0 && StringUtils.length(currentVal) > 4){//일련번호
					...
					if(tmpMmap == null){
						...
						param.put("inDdlyYN","0");
						
						rtn = orderOrdersDAO.searchWRS("code", naWrsC, tmpMmap.get("CSER_NA_TRPL_C").toString(), tmpMmap.get("CSER_NA_TEAM_C").toString()
								, tmpMmap.get("PRGR_C").toString(), "0", tmpMmap.get("SPLR_NA_TRPL_C").toString()
								,  String.valueOf(inDs.getObject(i, 4)), tmpMmap.get("CSER_CTR_TPC").toString(),"" , param);
