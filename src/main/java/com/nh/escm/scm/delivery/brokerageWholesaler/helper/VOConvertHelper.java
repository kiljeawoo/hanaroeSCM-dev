...
public class VOConvertHelper {
	...
	public static VOConvertHelper newInstance(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, DeliveryNoticeBrokerageWholesalerService deliveryNoticeBrokerageWholesalerService, SequenceKeyService seqKeyService, String rogoInfCrtDsc, String editType){
		return new VOConvertHelper(requestHelper, dsM, dsD, deliveryNoticeBrokerageWholesalerService, seqKeyService, rogoInfCrtDsc, editType);
	}
	...
	//배송예정서 일반등록
	public VOConvertHelper(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, DeliveryNoticeBrokerageWholesalerService deliveryNoticeBrokerageWholesalerService, SequenceKeyService seqKeyService, String rogoInfCrtDsc, String editType) {
		this.requestHelper = requestHelper;
		this.dsM = dsM;
		this.dsD = dsD;
		this.seqKeyService = seqKeyService;
		this.deliveryNoticeBrokerageWholesalerService = deliveryNoticeBrokerageWholesalerService;
				...
				if(StringUtils.equals(tempYn, "N")){//완료
					if(StringUtils.equals(DVY_PLASH_KDC_02, rogoInfCrtDsc)){//02 : 덤발주, 01 : 일반발주
						...
						for(int i = 0; i < 2; i++){
							...
							naDvyPlashSlpno = deliveryNoticeBrokerageWholesalerService.getNaDvyPlashSlpno(trplC, teamC, dvyPlaDt, ddlyYn);
	...
	private DeliveryNoticeOrderVO updateDeliveryNoticeOrder(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD){
		...
		for(int i = 0; i < dsD.getRowCount(); i++){
			...
			if(StringUtils.equals(dsD.getString(i, "DEL_YN"), "1") == true){
				...
				*/
				...
				double dvyVat = 0;
				if(StringUtils.equals("1", nodVo.getTXT_DSC())){
					dvyVat = Double.valueOf(deliveryNoticeBrokerageWholesalerService.getDvyVat(nodVo.getDVY_PLA_QT(), nodVo.getDVY_WRS_UPR(), nodVo.getTXT_DSC()));
				}
				...
				dnodVo.setRMK_CNTN(mvo.getRMK_CNTN());//비고
		...
		if(mvo.getBUY_UPR_DSC() != null){
			dnVo.setBUY_UPR_DSC(mvo.getBUY_UPR_DSC());
		}
		if("5".equals(mbcoType)){
			dnVo.setTRU_AGCY_NA_TRPL_C(requestHelper.getGlnCode());
		}
		return dnVo;
	}
	

	private DeliveryNoticeOrderVO insertDeliveryNoticeOrder(PlatFormRequestHelper requestHelper, DataSet dsM, DataSet dsD, String naDvyPlashSlpno, String dvyPlashKdc, String editType) throws AppetizerException{
		try{
			...
			throw new AppetizerException(e.getMessage());
		}
	}
}