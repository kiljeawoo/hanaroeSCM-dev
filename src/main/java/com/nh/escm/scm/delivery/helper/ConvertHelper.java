...
public class ConvertHelper {
	...
	DeliveryNoticeDAO deliveryNoticeDao = BeanUtil.getBean(DeliveryNoticeDAO.class);
	OrderOrdersDAO orderOrdersDAO = BeanUtil.getBean(OrderOrdersDAO.class);
	
	public static ConvertHelper newInstance(DataSet inDs){
		return new ConvertHelper(inDs);
	}
	
	public ConvertHelper(DataSet inDs){
		this.inDs = inDs;
	}
...
//			 String nextNadvySlpnoSeq = StringUtils.defaultString(String.valueOf(inDs.getObject(i+1, 39)));
	...
	public List<DataSet> excelValidationChk(PlatFormRequestHelper requestHelper, DataSet inDs) {
		return excelValidationChk(requestHelper, inDs, "0");
	}
	
	/*
	 * EXCEL Column Validation Check
	 */
	public List<DataSet> excelValidationChk(PlatFormRequestHelper requestHelper, DataSet inDs, String ddlyYn) {
		...
		for(int i = 0; i < inDs.getRowCount(); i++){
			List<OrderOrdersWrsVO> rtn = null;
			String rtnAmt = "";
			Map<String, Object> tmpMmap = null;
			
			String spyplNaTrplC = String.valueOf(inDs.getObject(i, 2)); // 공급처GLN
			String spyplNaTeamC = String.valueOf(inDs.getObject(i, 3)); // 공급처 팀코드
			if ("1".equals(ddlyYn)) {
				// 직송일 때는 수주처 값을 사용
				spyplNaTrplC = String.valueOf(inDs.getObject(i, 8));
				spyplNaTeamC = String.valueOf(inDs.getObject(i, 9));
			}
			
			List<Map<String, Object>> tmpList = orderOrdersDAO.getTrnrel(String.valueOf(inDs.getObject(i, 6)), String.valueOf(inDs.getObject(i, 7)), spyplNaTrplC
					, spyplNaTeamC, String.valueOf(inDs.getObject(i, 18)));
			
			if(tmpList != null && tmpList.size() > 0){
				tmpMmap = (Map<String, Object>)tmpList.get(0);
			}
			for(int j = 0; j < inDs.getColumnCount(); j++){
				logger.debug("[ inDs.getObject(i, j)] : " + inDs.getObject(i, j) );
				logger.debug("[ inDs.getObject(i, j).toString()] : " + inDs.getObject(i, j).toString());
				...
				if(j == 0 && StringUtils.length(currentVal) > 4){//일련번호
					...
					ds.set(row, "MSG" , msg);
					groupIdx[i] = StringUtils.defaultString(String.valueOf(inDs.getObject(i, 63)));
				}else if(j == 4 && DateUtil.getDifferDate(DateUtil.convertDateToString("yyyyMMdd", DateUtil.getCurrentDate()), currentVal) > 15){
					row = ds.newRow();
					msg = "라인번호[" + (i + 1) + "] "+ (j + 1)+"번째 Cell 값 [" + currentVal + "] 배송예정일은 현재날짜부터 15일 이내로 작성하세요.";
					ds.set(row, "ROW_NUMBER" , i + 1);		
					ds.set(row, "COLUMN_NUMBER" , j + 1);	
					ds.set(row, "MSG" , msg);
					groupIdx[i] = StringUtils.defaultString(String.valueOf(inDs.getObject(i, 63)));
					...
					//상품 확인
					String naWrsC = String.valueOf(inDs.getObject(i, 40));
					Map<String, Object> param = new HashMap<String, Object>();
					
					if(tmpMmap == null){
						row = ds.newRow();
						msg = "라인번호[" + (i + 1) + "] "+ (j + 1)+"번째는 거래관계가 잘못되었습니다";
						ds.set(row, "ROW_NUMBER" , i + 1);		
						ds.set(row, "COLUMN_NUMBER" , j + 1);	
						ds.set(row, "MSG" , msg);
						groupIdx[i] = StringUtils.defaultString(String.valueOf(inDs.getObject(i, 63)));
						...
						param.put("inBsDay","");
						param.put("inTrplR",tmpMmap.get("SPLR_NA_TRPL_C").toString());
						param.put("inTeamR",tmpMmap.get("SPLR_NA_TEAM_C").toString());
						param.put("inTrplO",tmpMmap.get("CSER_NA_TRPL_C").toString());
						param.put("inTeamO",tmpMmap.get("CSER_NA_TEAM_C").toString());
						param.put("inTrplM",tmpMmap.get("MNGT_NA_TRPL_C").toString());
						param.put("inTeamM",tmpMmap.get("MNGT_NA_TEAM_C").toString());
						param.put("inCtrTpc",tmpMmap.get("CSER_CTR_TPC").toString());
						param.put("inSpyTpc",tmpMmap.get("SPY_TPC").toString());
						param.put("inTrTpcM",tmpMmap.get("MNGT_NA_TR_TPC").toString());
						param.put("inWrsC","");
						param.put("inYear","0000");
						param.put("inUseSysR",tmpMmap.get("SPLR_NA_USR_SYS_KDC").toString());
						param.put("inUseSysO",tmpMmap.get("CSER_NA_USR_SYS_KDC").toString());
						param.put("inUseSysM",tmpMmap.get("MNGT_NA_USR_SYS_KDC").toString());
						param.put("inDdlyYN", ddlyYn);
						
						String orginTrplS = "";
						if ("1".equals(ddlyYn)) {
							orginTrplS = requestHelper.getGlnCode();
						}
						
						rtn = orderOrdersDAO.searchWRS("code", naWrsC, tmpMmap.get("CSER_NA_TRPL_C").toString(), tmpMmap.get("CSER_NA_TEAM_C").toString()
								, tmpMmap.get("PRGR_C").toString(), ddlyYn, tmpMmap.get("SPLR_NA_TRPL_C").toString()
								,  String.valueOf(inDs.getObject(i, 4)), tmpMmap.get("CSER_CTR_TPC").toString(), orginTrplS, param);
						
						
						if(rtn == null || rtn.size() == 0){
							row = ds.newRow();	
							msg = "라인번호[" + (i + 1) + "] "+ (j + 1)+"번째 Cell 값 [" + currentVal + "] 은 취급상품이 아니거나 삭제 된 상품번호입니다.";
							ds.set(row, "ROW_NUMBER" , i + 1);		
							ds.set(row, "COLUMN_NUMBER" , j + 1);	
							ds.set(row, "MSG" , msg);
							groupIdx[i] = StringUtils.defaultString(String.valueOf(inDs.getObject(i, 63)));
						}else if(!rtn.get(0).getNA_WRS_C().equals(naWrsC)){
							row = ds.newRow();	
							msg = "라인번호[" + (i + 1) + "] "+ (j + 1)+"번째 Cell 값 [" + currentVal + "] 은 취급상품이 아니거나 삭제 된 상품번호입니다.";
							ds.set(row, "ROW_NUMBER" , i + 1);		
							ds.set(row, "COLUMN_NUMBER" , j + 1);	
					...
					if(tmpMmap != null){
						...
						tmpMmap.put("NA_WRS_C" ,String.valueOf(inDs.getObject(i, 40)));
						tmpMmap.put("DVY_PLA_DT" ,String.valueOf(inDs.getObject(i, 4)));
						tmpMmap.put("DDLY_YN", ddlyYn);
						rtnAmt = deliveryNoticeDao.getSPnAdPwRSPRuT(tmpMmap);
						
						if(!rtnAmt.equals(String.valueOf(inDs.getObject(i, 50)))){
							row = ds.newRow();
							msg = "라인번호[" + (i + 1) + "] "+ (j + 1)+"번째 Cell 값 [" + currentVal + "] 은 단가[" + rtnAmt + "] 되어야 합니다.";
							ds.set(row, "ROW_NUMBER" , i + 1);		
							ds.set(row, "COLUMN_NUMBER" , j + 1);	
							ds.set(row, "MSG" , msg);
							groupIdx[i] = StringUtils.defaultString(String.valueOf(inDs.getObject(i, 63)));
