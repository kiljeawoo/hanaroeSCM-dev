(function()
{
    return function()
    {
        ...
        // User Script
        this.addIncludeScript("VAN_DS_SC_1543.xfdl", "lib::comLib.xjs");
        this.registerScript("VAN_DS_SC_1543.xfdl", function(exports) {
        ...
        // 저장용
        var save_params = ""; //파라미터
        var save_sSvcID = ""; //통신아이디
        var save_sURL = "";// 호출할 서버 페이지 주소
        var save_sInDatasets = "";//보내는데이터셋
        ...
        this.btn_save00_onclick = function(obj,e)
        {
        	...
        	if (application.confirm(msg)) {
        		...
        		// params 황목 최종 점검 후 진행
        		save_params += " rdo_sep="+reg_dsc //this.rdo_search_sep.value
        				  +" chk_temp_yn=0"//+this.chk_TEMP_YN.value
        				  +" chk_del_yn=0"//+this.chk_DELETE_YN.value;
        				  +" CTCPL=" + this.edt_ctcpl_telno.value;
        		
        		save_sSvcID        = "p_save";//통신아이디
        		save_sURL          = "svc::rest/pds/pricat/Product/insertNewGoodsbyKornet";// 호출할 서버 페이지 주소
        		//온라인 여부에 따라 이곳 데이터셋 구성을 다르게
        		let on_or_off = this.ds_wrs_rg_req.getColumn(0,"ONL_OBJ_YN");
        		console.log(on_or_off);
        		
        		if(on_or_off==1){
        			save_sInDatasets = 
        			"in_mainds=ds_wrs_rg_req_filter:A in_upload=ds_uploadsave:A ds_wrs_nfty_hdng=ds_wrs_nfty_hdng ds_namo_editor=ds_namo_editor";
        			//보내는데이터셋(온라인)
        		}
        		else{
        			save_sInDatasets = "in_mainds=ds_wrs_rg_req_filter:A in_upload=ds_uploadsave:A";//보내는데이터셋 (오프라인)	  
        		}
        		
        		if(this.rdo_store_code06.value == "1"){
        			this.CHK_BODY = false;
        			this.CHK_IMG = false;
        			this.wb_namo.callMethod("getEditorInput", "Y"); // 크로스에디터 입력 값 체크
        		}
        		
        		var sSvcID        = save_sSvcID;                                                            //통신아이디
        		var sURL          = save_sURL;                               // 호출할 서버 페이지 주소
        		var sInDatasets   = save_sInDatasets;//보내는데이터셋
        		var sOutDatasets  = "";
        		var sArgument     = save_params;                                                                       // 파라미터
        		var sCallbackFunc = "fn_callback";                                                           //콜백
        		var tranType 	  = "U"; // U:저장/삭제, S:조회
        		//API 호출
        		this.gfn_callService(sSvcID, sURL, sInDatasets, sOutDatasets, sArgument, sCallbackFunc, tranType);
        ...
        this.wb_namo_onusernotify = function(obj,e)
        {
        	...
        	if (!this.gfn_isNull(result)) {
        		...
        		trace("wb_namo_onusernotify===============>"+key);
        		...
        		if (key == "#INIT@") {
        			...
        			trace("this.NAMO_DATA===============>"+this.NAMO_DATA);
        			...
        			trace("this.NAMO_IMGSIZE===============>"+this.NAMO_IMGSIZE);
        ...
        this.fn_save = function()
        {
            ...
            this.fn_setSaveData(); // 온라인 데이터 셋팅
        	/*
        	var sSvcID        = save_sSvcID;                                                            //통신아이디
        	var sURL          = save_sURL;                               // 호출할 서버 페이지 주소
        	var sInDatasets   = save_sInDatasets;//보내는데이터셋
        ...
        this.loadIncludeScript("VAN_DS_SC_1543.xfdl", true);
