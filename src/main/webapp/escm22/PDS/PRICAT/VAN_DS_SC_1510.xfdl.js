    ...
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            if (Form == this.constructor) {
                this.set_name("VAN_DS_SC_1510");
                this.set_titletext("상품 상세 조회");
                this._setFormPosition(0,0,803,500);
            }
            ...
            obj = new Static("sta_title01", "absolute", "0", null, "709", "35", null, "5", this);
            obj.set_taborder("12");
            obj.set_text("- 서버 부하 문제로 리포트 출력 시 1,000건, 엑셀 다운로드 시 10,000건으로 조회 건수를 제한합니다.\r\n- 엑셀 다운로드 10,000건 이상 조회 시 문의 바랍니다.");
            this.addChild(obj.name, obj);
            ...
            //-- Default Layout
            obj = new Layout("default", "", 803, 500, this,
            	//-- Layout function
            	function(p) {
            		p.set_classname("VAN_DS_SC_1510");
            		p.set_titletext("상품 상세 조회");
        ...
        // User Script
        this.addIncludeScript("VAN_DS_SC_1510.xfdl", "lib::comLib.xjs");
        this.registerScript("VAN_DS_SC_1510.xfdl", function(exports) {
        ...
        this.search = function()
        {
        	...
        	if (edt00 == "" && edt01 == "") {
        		this.alert("사업장코드 또는 상품코드를 입력해주세요.");
        		return;
        	} else if (edt00 == "" && edt01.length < 7) {
        		alert("사업장코드 7자리 이상 입력해주세요.");
        		return;
        	} else if (edt01 == "" && edt00.length < 2) {
        	    alert("상품코드 2자리 이상 입력해주세요.");
        	    return;
        	}
        	...
        	if (date1 != "" && date2 != "") {
        		...
        		this.alert(this.gfn_getTextMessage('msg.van_ds_sc_1510.view.transaction.history')+' ['+date1+' - '+date2+']');
        		return;
        	} else if (date1 == "" && date2 != "") {
        		this.alert(this.gfn_getTextMessage('msg.van_ds_sc_1510.view.transaction.history')+' ['+date1+' - '+date2+']');
        		return;
        ...
        {
        	if (params == true) {
        		var bzplc = this.gfn_nullToEmpty(this.div_search.edt_data02.value);
        		var bzplcNm = this.gfn_nullToEmpty(this.div_search.edt_data03.value);
        ...
        {
        	if (params == true) {
        		if (this.gfn_nullToEmpty(this.div_search.edt_data00.value) != "" && this.gfn_nullToEmpty(this.div_search.edt_data00.value).length < 2) {
        ...
        this.loadIncludeScript("VAN_DS_SC_1510.xfdl", true);
