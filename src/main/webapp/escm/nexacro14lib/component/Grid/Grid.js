...
if (!nexacro.Grid) {
		...
		if (!value) {
			...
			for (var i = _treeIndexes.length - 1; i >= 0; i--) {
				...
				if (cellinfo) {
					...
					if (!state || state == "") {
						if (expand) {
							if (this._setTreeState(i, 1, false, true) > 0) {
								i = _treeIndexes.length;
								update = true;
							}
						}
						else {
							if (this._setTreeState(i, 0, false, true) > 0) {
								i = _treeIndexes.length;
								update = true;
							}
						}
						...
						var s = this._setTreeState(i, state, false, true);
						if (s == 2) {
							i = _treeIndexes.length;
							update = true;
						}
	...
	_pGrid._setTreeState = function (rowidx, v, redraw, not_change_value) {
		v = parseInt(v, 10);
		...
		if (isFinite(v)) {
			...
			if (v != state) {
				...
				else if ((retn = this._toggleTreeState(rowidx, redraw, v, not_change_value)) > 0) {
					return retn;
				}
			}
		}
	...
	_pGrid._toggleTreeState = function (rowidx, redraw, v, not_change_value) {
		var dsrowidx = this._treeIndexes[rowidx];
		var state = this._treeStates[dsrowidx];
		var collapse = false;
		var retn;
		...
		else if (state == 2 && !not_change_value) {
			if (this.on_fire_cantreestatuschange(rowidx, dsrowidx, 0) !== false) {
				retn = 1;
