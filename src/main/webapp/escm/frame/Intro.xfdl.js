(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj.set_loadfiltermode("keep");
            obj.set_reversesubsum("false");
            obj._setContents("<ColumnInfo><Column id=\"USR_TPC\" type=\"STRING\" size=\"256\"/><Column id=\"USR_TPNM\" type=\"STRING\" size=\"256\"/></ColumnInfo><Rows><Row><Col id=\"USR_TPNM\">- 회원유형 선택 -</Col><Col id=\"USR_TPC\">00</Col></Row><Row><Col id=\"USR_TPC\">01</Col><Col id=\"USR_TPNM\">협력업체</Col></Row><Row><Col id=\"USR_TPC\">02</Col><Col id=\"USR_TPNM\">본부</Col></Row><Row><Col id=\"USR_TPNM\">사업장</Col><Col id=\"USR_TPC\">03</Col></Row><Row><Col id=\"USR_TPNM\">세금계산서전용</Col><Col id=\"USR_TPC\">04</Col></Row><Row><Col id=\"USR_TPC\">05</Col><Col id=\"USR_TPNM\">전자계약전용</Col></Row><Row><Col id=\"USR_TPC\">20</Col><Col id=\"USR_TPNM\">샘플20</Col></Row><Row><Col id=\"USR_TPC\">30</Col><Col id=\"USR_TPNM\">샘플30</Col></Row><Row><Col id=\"USR_TPC\">81</Col><Col id=\"USR_TPNM\">농협몰</Col></Row></Rows>");
            this.addChild(obj.name, obj);
            ...
            this.div_mainbg.div_main_visual.div_user_tab.addChild(obj.name, obj);
            obj = new Button("btn_cancel", "absolute", "15", null, "134", "35", null, "10", this.div_mainbg.div_main_visual.div_user_tab.div_tab_join);
            obj.set_taborder("19");
            obj.set_text("취소");
            obj.set_cssclass("btn_INTRO_cancel");
            this.div_mainbg.div_main_visual.div_user_tab.div_tab_join.addChild(obj.name, obj);
            obj = new Button("btn_confirm", "absolute", null, null, "134", "35", "15", "10", this.div_mainbg.div_main_visual.div_user_tab.div_tab_join);
            obj.set_taborder("20");
            obj.set_text("확인");
            obj.set_cssclass("btn_INTRO_confirm");
            this.div_mainbg.div_main_visual.div_user_tab.div_tab_join.addChild(obj.name, obj);
            obj = new Combo("cmb_usr_tpc", "absolute", "15", "15", "200", "33", null, null, this.div_mainbg.div_main_visual.div_user_tab.div_tab_join);
            this.div_mainbg.div_main_visual.div_user_tab.div_tab_join.addChild(obj.name, obj);
            obj.set_taborder("25");
            obj.set_codecolumn("USR_TPC");
            obj.set_datacolumn("USR_TPNM");
            obj.set_displayrowcount("10");
            obj.style.set_cursor("hand");
            obj.set_innerdataset("@ds_REG_USR_TPC");
            obj.set_value("00");
            obj.set_text("회원유형 선택");
            obj.set_index("0");
        ...
        // User Script
        this.addIncludeScript("Intro.xfdl", "lib::comLib.xjs");
        this.registerScript("Intro.xfdl", function() {
        /************************************************************************************************
         * 공통 라이브러리 INCLUDE 영역
        ************************************************************************************************/
        ...
        ************************************************************************************************/
        			...
        			case "retrieveSession":
        				...
        				if (this.ds_login.rowcount > 0) {
        					...
        					if (application.gv_userTPC == "user") {
        						...
        						trace("this.ds_login.getColumn(0,commanyType)=====>"+this.ds_login.getColumn(0,"commanyType"));
        						trace("application.gds_session.getColumn(0,commanyType)=====>"+application.gds_session.getColumn(0,"commanyType"));
        						trace("application.gv_companyType=====>"+application.gv_companyType);
        						if(this.ds_login.getColumn(0,"commanyType")=='1040'){
        							this.fn_callAuthorityWorkplaceLiq();
        						}
        ...
        /*
        //  	 주류공급처의 권한매장의 경우 처음에는 gds_session이 null인 상태 이므로 새로고침을
        //  	 한번 해주어 세션값을 글로벌 변수(gds_session)에 담아준다.
        	console.log(application.gds_session);
        	console.log(application.gds_session._rawRecords.length);
         	if(application.gds_session._rawRecords.length==0 && application.gv_glnCode!=''){
        		location.reload();
        	}
        */
        }
        ...
        ************************************************************************************************/
        ...
        this.btn_new_onclick = function(obj,e)
        {
        	...
        	this.gfn_OpenMenuId("98010000", "", "BT");
        }
        ...
        ****************************************************/
        this.btn_confirm_onclick = function(obj,e)
        {
        	...
        	/*
        	 *
        	var sVal =  tab_join.rdo_userType1.value;
        		sVal += tab_join.rdo_userType2.value;
        		sVal += tab_join.rdo_userType3.value;
        		sVal += tab_join.rdo_userType4.value;
        		sVal += tab_join.rdo_userType5.value;
        	*/
        	...
        	// 회가인 1. 회원의 유형을 선택하는 부분
        	 //본부와 사업장만 테스트 범위이다.
        	 //fn_joinUser 를 통하여 회원인증 팝업창을 연다.
        	// 본부 사업장의 경우는 인증을 먼저 한다는 차이점이 있다.
        	if (sVal == "01") {
        		fn_joinUser_params={
        			popup_page : "OMG.PT::OMG_DS_SC_5110.xfdl",
        			usrTpc : '01',
        			user_group_dscription : 'unknown'
        		}
        		this.fn_joinUser(fn_joinUser_params);
        	} else if (sVal == "02") {// 본부일 때
        		//this.fn_joinUser("OMG.PT::OMG_DS_SC_5111.xfdl");
        		fn_joinUser_params={
        			popup_page : "POPUP::POPUP_SELF_AUTH.xfdl",
        			usrTpc: '02',
        			user_group_dscription : 'head_quater'
        		}
        		this.fn_joinUser(fn_joinUser_params);
        	} else if (sVal == "03") {// 사업장일 때
        		//this.fn_joinUser("OMG.PT::OMG_DS_SC_5112.xfdl");
        		fn_joinUser_params={
        			popup_page : "POPUP::POPUP_SELF_AUTH.xfdl",
        			usrTpc : '03',
        			user_group_dscription : 'oneway_mart'
        		}
        		this.fn_joinUser(fn_joinUser_params);
        	} else if (sVal == "04") {
        		fn_joinUser_params={
        			popup_page : "OMG.PT::OMG_DS_SC_5117.xfdl",
        			usrTpc : '04',
        			user_group_dscription : 'tax_bill'
        		}
        		this.fn_joinUser(fn_joinUser_params);// 세금계산서전용 추가
        	} else if (sVal == "05") {
        		fn_joinUser_params={
        			popup_page : "OMG.PT::OMG_DS_SC_5180.xfdl",
        			usrTpc : '05',
        			user_group_dscription : 'E_contract'
        		}
        		this.fn_joinUser(fn_joinUser_params);// 전자계약전용 추가
        	} else if (sVal == "20") {
        		fn_joinUser_params={
        			popup_page : "OMG.PT::OMG_DS_SC_5190.xfdl",
        			usrTpc : '20',
        			user_group_dscription : 'sample_20'
        		}
        		this.fn_joinUser(fn_joinUser_params);// sample_20 추가
        	} else if (sVal == "30") {
        		fn_joinUser_params={
        			popup_page : "OMG.PT::OMG_DS_SC_5191.xfdl",
        			usrTpc : '30',
        			user_group_dscription : 'sample_30'
        		}
        		this.fn_joinUser(fn_joinUser_params);// sample_30 추가
        	} else if (sVal == "81") {
        		fn_joinUser_params={
        			popup_page : "OMG.PT::OMG_DS_SC_5170.xfdl",
        			usrTpc : '81',
        			user_group_dscription : 'AMARKET'
        		}
        		this.fn_joinUser(fn_joinUser_params);// AMARKET 추가
        	}
        }
        ...
        /***************************************************
        	회원종류에 따른 회원가입 화면 호출
        ****************************************************/
        //회가인 2. 회원인증 팝업창을 여는 곳
        //신경쓸것이 별로 없을 것이다.
         this.fn_joinUser = function(fn_joinUser_params)
        {
        	//this.btn_close_onclick();
        	this.div_mainbg.div_main_visual.div_user_tab.btn_tab_login.click();
        	//set url
        	var url = fn_joinUser_params.popup_page;
        	var oArg = {};
        	let sPopupId ='';
        	// if fn_joinUser_params.user_group is '02'or '03' add user_group in oArg
        	//02=head_quater, 03=oneway_mart
        	if (fn_joinUser_params.usrTpc == '02' || fn_joinUser_params.usrTpc == '03') {
        		oArg = {
        			paramMode: "U", dsArg: "",
        			//user_group: fn_joinUser_params.user_group,
        			//user_group_dscription: fn_joinUser_params.user_group_dscription
        		};
        		sPopupId = 'auth_popup';
        		//localStorage.setItem("user_group", fn_joinUser_params.user_group);
        	} else {
        	...
        	var sOption = "autosize=true, title=true";
        	var sPopupCallBack = "fn_popupAfter";
        	this.gfn_openPopup(sPopupId, url, oArg, sOption, sPopupCallBack);
        }
        ...
        ****************************************************/
        ...
        {
        	//회가인 6. 선택된 사용자 유형에 따라 알맞은 팝업창을 열어주는 코드
        	//그외 중효한 것은 인증페이지에서 받아온 정보를 localstorage에 저장하는 정도이다.
        	if (strId = 'auth_popup') {
        		console.log(fn_joinUser_params);
        		if (fn_joinUser_params.usrTpc == '02') {// 본부인경우(처음 가입유형 선택시 저장된 값)
        			strVal = JSON.parse(strVal);
        			console.log(fn_joinUser_params.usrTpc);
        			console.log(strId);
        			console.log(strVal);
        			
        			//save auth result in localStorage.
        			localStorage.setItem('member_join_info', JSON.stringify(strVal));
        			

        			if(strVal.ds_api_result_obj.tgrmRspC=='0'){//0번이 정상
        				//OMG.PT::OMG_DS_SC_5112.xfdl 팝업오픈
        				let sPopupId='popId';
        				let url = 'OMG.PT::OMG_DS_SC_5111.xfdl';
        				var oArg = {paramMode: "U", dsArg: ""};
        				var sOption = "autosize=true, title=true";
        				var sPopupCallBack = "HQ_fn_popupAfter";
        				this.gfn_openPopup(sPopupId, url, oArg, sOption, sPopupCallBack);
        			}else{
        				alert("본부 인증에 실패하였습니다. 다시 시도해 주세요.");
        			}
        		} else if (fn_joinUser_params.usrTpc == '03') {// 사업장인경우(처음 가입유형 선택시 저장된 값)
        			strVal = JSON.parse(strVal);
        			console.log(fn_joinUser_params.usrTpc);
        			console.log(strId);
        			console.log(strVal);
        			...
        			if(strVal.ds_api_result_obj.tgrmRspC=='0'){//0번이 정상
        				//OMG.PT::OMG_DS_SC_5112.xfdl 팝업오픈 
        				let sPopupId='popId';
        				let url = 'OMG.PT::OMG_DS_SC_5112.xfdl';
        				var oArg = {paramMode: "U", dsArg: ""};
        				var sOption = "autosize=true, title=true";
        				var sPopupCallBack = "OM_fn_popupAfter";
        				this.gfn_openPopup(sPopupId, url, oArg, sOption, sPopupCallBack);
        			}else{
        				alert("사업장 인증에 실패하였습니다. 다시 시도해 주세요.");
        			}
        		} else {
        			console.log(strId);
        			console.log(strVal);
        			//alert("본부도 사업장도 아닙니다. 소스를 확인하시오.")
        		}
        	}
        }
        ...
        //본부팝업 후처리 소스작성
        this.HQ_fn_popupAfter=function(){
        	
        }

        
        //사업장팝업 후처리 소스작성
        this.OM_fn_popupAfter=function(){
        ...
        this.fn_changeUserType = function(obj,e)
        {
        	...
        	for (var i=1; i<6; i++) {
        		if (sCode == i) continue;
        		this.div_mainbg.div_main_visual.div_user_tab.div_tab_join.all["rdo_userType"+i].set_value(""); // 각각 별도의 innerdataset 을 쓰고 있어서 이렇게 초기화 함.
        ...
        this.loadIncludeScript("Intro.xfdl");
